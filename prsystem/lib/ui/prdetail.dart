...
import 'package:prsystem/modal/PRApproveList.dart';
import 'package:prsystem/modal/mGetapprover.dart';
import 'package:prsystem/modal/mLogPrNumber.dart';
...
import 'package:prsystem/ui/prapprovedAC.dart';
...
import 'package:prsystem/ui/prPDFFile.dart';

class prDetail extends StatelessWidget {
  String PRNumber;
  String type;
  prDetail({required this.PRNumber, required this.type});
...
class _prDetailScreenState extends State<prDetailScreen> {
  ...
  void openattachfile(String attachfile, String types) {
    if (attachfile != "") {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Can\'t open Attach File..'),
        ),
      );
    }
  }
      ...
      body: Container(
        child: Material(
          child: Column(
            children: [
              Expanded(
                child: Consumer<PRListProvider>(builder:
                    (BuildContext context, PRListProvider provider, _) {
                  return Column(
                    children: [
                      Expanded(
      ...
      child: Expanded(
        ...
        itemBuilder: (BuildContext context, int index) {
          ...
          if (_detailss.types != "T") {
                            ...
                            Row(
                              ...
                              children: [
                                Column(
                                  ...
                                  children: [
                                    TestHead('${_detailss.qty}', Colors.grey,
                                        FontWeight.normal),
                                    TestHead(
                            ...
                            Container(
                              child: SizedBox(
                                ...
                                child: new IconButton(
                                  ...
                                  iconSize: _large ? 25 : (_medium ? 20 : 18),
                                  onPressed: () {
                                    showuploadimage(
            ...
            return Container(
              child: TestHead(
                  '  >> ${_detailss.textFree}', Colors.grey, FontWeight.normal),
            );
          }
        },
      )),
      ...
      if (user.empEmail != null) {
        ...
        if (getapprover.length > 0) {
          ...
          if (_data.levels == "1") {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Can\'t access this Menu..'),
              ),
            );
          } else {
            Navigator.push(
